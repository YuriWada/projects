...
void shellSort(int *A, int n, sortperf_t *s)
{
    ...
    while (h >= 1)
    {
        for (int i = h; i < n; i++)
        {
            for (int j = i; j >= h && (inccmp(s, 1), A[j] < A[j - h]); j -= h)
            {
                swap(&A[j], &A[j - h], s);
            }
        }
...
void selectionSort(int arr[], int l, int r, sortperf_t *s)
{
    ...
    for (int i = l; i < r; i++)
    {
        ...
        if (min_idx != i)
        {
            swap(&arr[i], &arr[min_idx], s); // Realiza a troca e incrementa movimentos
        }
    }
}
...
{
    inccalls(s, 1); // Incrementa chamadas no início
    ...
    for (int i = l + 1; i <= r; i++)
    {
        ...
        while (j >= l && v[j] > key)
        {
            v[j + 1] = v[j]; // Movimenta o elemento para a direita
            incmove(s, 1);   // Incrementa movimentações
            j--;

            // Comparação adicional no `while`
            if (j >= l)
                inccmp(s, 1);
        }
        ...
        v[j + 1] = key;
        incmove(s, 1); // Incrementa movimentação para inserir o `key`

        // Comparação final ao sair do `while`
        if (j < l || v[j] <= key)
            inccmp(s, 1);
    }
}
...
void partition3(int *A, int l, int r, int *i, int *j, sortperf_t *s)
{
    ...
    *j = r;
    ...
    while (*i <= *j)
    {
        while ((inccmp(s, 1), A[*i] < pivot))
            (*i)++;
        while ((inccmp(s, 1), A[*j] > pivot))
            (*j)--;
...
int main(int argc, char **argv)
{
    ...
    int retp = clock_gettime(CLOCK_MONOTONIC, &inittp);
